...
The read happens on the parent's `ocr_worker_thread` (separate thread from the Flask request handlers, but same process) just before `Process(...).start()`. A `bool` read in CPython is atomic, so no Lock is required — but a one-line comment in code makes intent clear for maintainers. **Note**: a subprocess already in flight does **not** pick up a toggle change mid-batch; the new value applies to the next subprocess spawn (typically within one cooldown period).
...
`ocr.py` exposes `extract_text_from_image(image: np.ndarray)` as the unchanged public dispatcher. Two private backend functions live in the same module: `_extract_with_doctr(image)` and `_extract_with_vision(image)` (the latter delegates to `apple_vision_ocr.extract_text_with_vision`).
...
ocr.extract_text_from_image(img)
  ...
  │    try:
  │       return _extract_with_vision(img)
...
The current `ocr.py` (lines 1–7) instantiates `ocr_predictor(...)` at module import time. **This eager instantiation is removed** as part of this work. In its place, `ocr.py` declares `_doctr_predictor = None` at module level and a `_get_doctr_predictor()` helper that constructs it on first call.

`_extract_with_doctr` calls `_get_doctr_predictor()` lazily. First invocation constructs `ocr_predictor(pretrained=True, det_arch="db_mobilenet_v3_large", reco_arch="crnn_mobilenet_v3_large")`. Subsequent calls reuse the cached predictor.
...
**Plus**: the existing unified settings handler `POST /api/settings` (around `app.py:3621`, used by the modal's "Save Changes" button at line ~1301) must also learn the new `use_apple_vision` key — read it from the request body, call `set_use_apple_vision`, and merge it into the persisted settings. Adding only the dedicated route without updating the unified handler would leave the modal's main save path inconsistent.
...
Three PNG fixtures in `tests/fixtures/apple_vision/`, generated by a small reproducible script `tests/fixtures/apple_vision/generate.py` (uses PIL with a known TrueType font shipped with macOS, e.g. `/System/Library/Fonts/Helvetica.ttc`, at known coordinates). The script is checked in alongside the PNGs so fixtures can be regenerated identically:
...
The umbrella `pyobjc==10.3` already pulls in `pyobjc-framework-Vision` and `pyobjc-framework-Quartz` transitively. The explicit pins are added for clarity and to fail fast in CI/build if the umbrella ever stops shipping them. They add two extra rows to `uv.lock` but no extra wheel downloads.
